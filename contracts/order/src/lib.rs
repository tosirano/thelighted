...
use soroban_sdk::{
    contract, contractclient, contractimpl, contracttype, symbol_short, vec, Address, Env, String,
    Vec,
};
...
pub struct Order {
    ...
    pub expires_at: u64,
    /// True once loyalty tokens have been minted for this order.
    pub minted: bool,
}
...
pub enum DataKey {
    ...
    RestaurantRegistry,
    /// Optional address of the LoyaltyToken contract for auto-minting.
    LoyaltyToken,
...
impl OrderContract {
    ...
    /// Set (or update) the loyalty token contract address (admin only).
    ...
    /// Expire an order that has passed its deadline. Callable by anyone.
    pub fn expire_order(env: Env, order_id: u64) {
        ...
        if order.status == OrderStatus::Cancelled || order.status == OrderStatus::Delivered {
            panic!("order already finalised");
        }
        order.status = OrderStatus::Cancelled;
        order.updated_at = env.ledger().timestamp();
        Self::save_order(&env, &order);
        env.events()
            .publish((symbol_short!("expired"), symbol_short!("order")), order_id);
    }
}
...
mod test {
    ...
    fn test_overflow_quantity_max_and_large_price() {
        let (env, client, _admin, _registry) = setup();
        let customer = Address::generate(&env);
        // quantity = u32::MAX, unit_price derived from make_item = 1, won't overflow,
        // but we use a custom item with huge price to trigger overflow
        let snap = MenuItemSnapshot {
            menu_item_id: 1,
            name: String::from_str(&env, ""),
            price_at_order: 0,
    ...
    fn test_valid_multi_item_order_calculates_correct_total() {
        ...
        let snap1 = MenuItemSnapshot {
            menu_item_id: 1,
            name: String::from_str(&env, ""),
            price_at_order: 5_000_000,
        };
        let snap2 = MenuItemSnapshot {
            menu_item_id: 2,
            name: String::from_str(&env, ""),
            price_at_order: 7_000_000,
        };
        let items = vec![
            &env,
            OrderItem {
        ...
        // 3 * 5_000_000 + 2 * 7_000_000 = 29_000_000
    ...
    fn test_order_exceeds_max_total_panics() {
        ...
        let snap = MenuItemSnapshot {
            menu_item_id: 1,
            name: String::from_str(&env, ""),
            price_at_order: MAX_ITEM_UNIT_PRICE,
        };
        let items = vec![
            &env,
            OrderItem {
                ...
                name: String::from_str(&env, ""),
                quantity: 101,
                unit_price: MAX_ITEM_UNIT_PRICE,
                snapshot: snap,
            },
        ];
        client.place_order(&customer, &1, &items, &String::from_str(&env, ""));
    ...
    fn test_unit_price_above_max_rejected() {
        ...
        let snap = MenuItemSnapshot {
            menu_item_id: 1,
            name: String::from_str(&env, ""),
            price_at_order: MAX_ITEM_UNIT_PRICE + 1,
        };
        let items = vec![
            &env,
            OrderItem {
                ...
                name: String::from_str(&env, ""),
                quantity: 1,
                unit_price: MAX_ITEM_UNIT_PRICE + 1,
                snapshot: snap,
            },
        ];
        client.place_order(&customer, &1, &items, &String::from_str(&env, ""));
    ...
    fn place_one(env: &Env, client: &OrderContractClient, _admin: &Address) -> u64 {
        let customer = Address::generate(env);
        let items = vec![env, make_item(env, 1, 1)];
        client.place_order(&customer, &1, &items, &String::from_str(env, ""))
    }
    ...
    fn test_expire_order_emits_event() {
        ...
        env.ledger()
            .with_mut(|l| l.timestamp = super::ORDER_TTL_SECONDS + 1);
        client.expire_order(&id);
        // expire_order succeeded and order is now cancelled
        assert_eq!(client.get_order(&id).status, OrderStatus::Cancelled);
    }
